...
namespace WebRTCTest.Hubs
{
    public class ConnectionHub : Hub<IConnectionHub>
    {
        ...
        public ConnectionHub(List<User> users, List<UserCall> userCalls, List<CallOffer> callOffers, UserManager<AppUser> userManager, SignInManager<AppUser> signInManager)
        {
            _Users = users;
        ...
        public async Task SendAudioONOFF(bool myAudioOnOff, User targetUser)
        {
            await Clients.Client(targetUser.ConnectionId).ReceiveAudioONOFF(myAudioOnOff);
        }

        public async Task UpdateUserInfo(UserInfo userinfo)
        {
            var user = await _UserManager.FindByEmailAsync(userinfo.UserEmail);
            user.ActiveStatus = userinfo.UserStatus;
        ...
        private async Task SendUserListUpdate()
        {
            ...
            try
            {
                if (_UserManager.Users != null)
                {
                    ...
                    var qry = allUsers.GroupJoin(
                              _Users,
                              a => a.Email,
                              u => u.Username,
                              (x, y) => new { All = x, Active = y })
                           .SelectMany(
                               x => x.Active.DefaultIfEmpty(),
                            (x, y) => new UserInfo
                            {
                                UserFullName = x.All.FullName,
                                GoogleProfile = x.All.GoogleProfile,
                                UserEmail = x.All.Email,
                                UserStatus = x.All.ActiveStatus,
                                ConnectionId = y?.ConnectionId,
                                InCall = y?.InCall??false
                            }).OrderBy(s => s.UserFullName);
                    var allUserList = qry.ToList();

                    await Clients.All.UpdateUserList(allUserList, _Users.Count());
                }
            }
            catch (Exception e)
            {
    ...
    {
        Task UpdateUserList(List<UserInfo> userList, int activeUserCount);
        ...
        Task ReceiveAudioONOFF(bool partnerAudioOnOff);
    }
}
